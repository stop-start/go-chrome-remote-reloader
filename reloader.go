...
type RemoteConfig struct {
	...
	ExecName string
	// DebugAddr is the chrome remote debugging address (default localhost)
	DebugAddr string
	// DebugPort is the chrome remote debugging port (default 9222)
	DebugPort int
...
func (rc *RemoteConfig) RemoteChrome() (context.CancelFunc, error) {
	...
	cmd := exec.CommandContext(
		ctx,
		rc.ExecName,
		fmt.Sprintf("--remote-debugging-port=%d", rc.DebugPort),
